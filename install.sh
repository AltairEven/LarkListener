...
if [ ! -f "$BINARY" ]; then
    echo "❌ lark-listener binary not found in $SCRIPT_DIR"
    exit 1
fi
...
if [ "$HAS_ERROR" = true ]; then
    echo ""
    echo "请先安装缺失的依赖，然后重新运行 ./install.sh"
    exit 1
fi

echo ""

# 3. Create directories and copy binary
...
if [ ! -f "$LISTENER_HOME/config.yaml" ]; then
    ...
    # Get bot_chat_id by sending a test message
    echo ""
...
CONF
    ...
    echo ""
    echo "✓ 配置文件已生成 → $LISTENER_HOME/config.yaml"
else
    echo "✓ 配置文件已存在，跳过（如需重新配置，请删除 $LISTENER_HOME/config.yaml 后重新安装）"
fi
...
echo ""
echo "后续步骤："
echo "  1. 编辑配置:  vim $LISTENER_HOME/config.yaml"
echo "  2. 启动服务:  launchctl load $PLIST_PATH"
echo "  3. 停止服务:  launchctl unload $PLIST_PATH"
echo "  4. 查看日志:  tail -f $LISTENER_HOME/logs/stderr.log"
echo ""
echo "使用方式："
echo "  • 服务会每隔 poll_interval 秒自动汇总消息"

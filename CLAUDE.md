...
| `config.py` / `config_editor.py` | 读/改 config.yaml（ruamel 保留注释；`ai`/`notify`/`lark_cli_appid` 受保护不可经 bot 改）；`load_config` 钳制 `poll_interval` 为非负 int（null/字符串/负数等坏值绝不让消费点 TypeError 崩进 KeepAlive 重启循环） |
